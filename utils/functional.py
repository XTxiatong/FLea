...
def client_inner_dirichlet_partition(targets, num_clients, num_classes, dir_alpha, client_sample_nums, verbose=True):
    ...
    if num_clients == 500 and dir_alpha == 0.1:
        THE = 460
    else:
        THE = 0
    ...
    class_priors = np.random.dirichlet(alpha=[dir_alpha] * num_classes, size=num_clients)
    ...
    # a list for sampless, hold by zeros (idx 0 will be reused)
    
    Flag = True
    while np.sum(client_sample_nums) != 0 and Flag:
        curr_cid = np.random.randint(num_clients)
        # If current node is full resample a client
        if verbose:
            print('Remaining Data: %d' % np.sum(client_sample_nums))
        if np.sum(client_sample_nums) < THE:
            Flag = False
        if client_sample_nums[curr_cid] <= 0: #This cid finised
            continue
